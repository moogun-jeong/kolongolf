# **TASK.md - 디자인 요소 보강 및 하단 일정 공지 팝업**

현재 미니멀 홈페이지 구조를 유지하면서 히어로, 일정, 카드 구간에 더 풍부한 디자인 요소를 추가하고, 모바일 앱 하단 광고처럼 노출되는 다음 일정 공지 팝업을 구현합니다.

## **1. 현재 진행 중인 작업 (Current Active Task)**
*   [x] 작업 맥락 학습: `AGENTS.md`, `TASK.md`, `PROJECT_LOG.md`, `GEMINI.md`, `blueprint.md` 확인
*   [x] 현재 디자인 검토: 밝은 서비스형 구조는 유지하되 히어로/일정 구간의 시각적 밀도 보강 필요 확인
*   [x] 디자인 요소 보강: 배경 질감, 히어로 정보 스트립, 일정 강조 패널, 카드 디테일 추가
*   [x] 하단 일정 공지 팝업 구현: 다음 스크린 행사 안내, 닫기, 상세 보기, 일주일간 열지 않기 체크박스 제공
*   [x] `localStorage` 기반 7일 숨김 동작 구현
*   [x] 브라우저 DOM 렌더링, 자산 응답, 데스크톱 레이아웃 검증
