...
### **[2026-08-10] 디자인 현대화 Phase 0~3 (다크 모드 제외)**

#### **왜 했는가**
정지 화면의 컨셉(딥그린·아이보리·골드, 세리프/산세리프 페어링)은 방향이 옳았으나 실행 밀도가 낮았습니다. 측정해 보니 토큰은 20개인데 하드코딩된 `oklch()` 리터럴이 159개(고유 127개), `@keyframes`와 `cubic-bezier`는 각각 0개, 트랜지션 16개가 전부 기본 `ease`였습니다. `container-type`을 전 섹션에 선언해두고 `@container` 규칙은 1개뿐이었습니다. 진단 전문은 `DESIGN_MODERNIZATION_PLAN.md`에 있습니다.

#### **토큰 시스템 (Phase 0~1)**
*   유동 타이포 스케일 8단계(`--text-2xs`~`--text-3xl`)를 `clamp()`로 신설. `font-size` 선언 108개를 전부 이관하고 가독 텍스트 하한을 14px로 올렸습니다. 기존에는 108개 중 44개가 12px 이하였고 `clamp()`는 3,800줄에서 3회뿐이라 761~1100px 구간에서 히어로 제목이 60px로 고정돼 있었습니다.
*   유동 스케일 도입 후 모바일 미디어 쿼리의 제목 축소 오버라이드 5개를 제거했습니다. 그대로 두면 `clamp` 축소와 오버라이드 축소가 겹쳐 이중으로 작아집니다.
*   색은 `color-mix(in oklab, ...)` 파생으로 바꿨습니다. 같은 골드의 투명도 변주를 다섯 군데에 손으로 다시 적던 것이 원인이었고, 리터럴을 159→74개로 줄였습니다. 남은 74개는 그라디언트·동물 아바타 색상처럼 실제로 일회성인 값입니다.
*   간격 61종을 4pt 기반 8단계로, 모서리 반경 12종을 3단계(`--r-sharp`/`--r-card`/`--r-pill`)로 정리했습니다. 반경은 버튼 2px와 카드 18px가 서로 다른 디자인 언어를 말하고 있어 브랜드에 맞는 샤프한 쪽으로 통일했습니다.
*   그림자는 접촉+환경 2겹 3단계(`--elev-1~3`)로 교체했습니다. 기존 14종은 사실상 같은 그림자를 미세하게 다르게 적어둔 것이었습니다.

#### **실제 버그 수정**
*   **`font-weight: 900` 30곳** — `index.html`이 Inter를 500/600/700/800만 로드하므로 900 페이스가 없어 브라우저가 가짜 볼드를 합성하고 있었습니다. 12px 대문자 키커가 탁해 보이던 원인입니다. `--weight-label`(650)로 교체했습니다.
*   **아카이브 카드 이미지** — 202px 높이 카드에 1800px `-display` 원본을 그대로 내려주고 있었습니다. 짝이 되는 `-thumb`(400px)이 이미 있어 `srcset`으로 연결했습니다. 두 자산 합계가 6,076KB 대 440KB입니다.
*   **모바일 가로 넘침** — `.hero-course-svg`가 `width: 118vw`를 `-38vw` 위치에 두고 있어 넘침이 설계상 확정이었고, `body { overflow-x: hidden }`이 이를 가리고 있었습니다.

#### **모션 (Phase 2)**
*   `motion` 캐스케이드 레이어를 신설하고 비대칭 감속 이징 4종을 도입했습니다.
*   리빌과 진행바를 `animation-timeline`(`view()` / `scroll(root block)`)으로 옮겼습니다. Firefox가 기본 비활성이므로 전부 `@supports`로 감싸고, `main.js`는 `CSS.supports("animation-timeline", "view()")`로 분기해 미지원 시에만 기존 IntersectionObserver 경로를 씁니다.
*   `initScrollProgress()`가 스크롤 이벤트마다 `scrollHeight`를 읽어 강제 리플로우를 일으키던 것을 지원 브라우저에서는 아예 실행하지 않게 했습니다(`HOMEPAGE_REVIEW.md` H-18). 폴백 경로도 `requestAnimationFrame`으로 프레임당 1회로 묶었습니다.
*   히어로 슬라이드 교체를 View Transitions로 바꿨습니다. 기존 120ms `setTimeout` 방식은 타이머와 실제 디코딩 시점이 어긋나 느린 회선에서 빈 프레임이 보였습니다. 미지원 브라우저용 폴백도 `Image.decode()` 후 교체하도록 고쳤습니다.
*   아카이브 카드 → 라이트박스 전환에 `view-transition-name`을 붙였습니다. 이름은 문서 내 유일해야 하므로 전환 콜백 안에서 썸네일 → 라이트박스 이미지로 넘기고 종료 시 해제합니다.
*   `prefers-reduced-motion`에서는 `animation-duration`만 0으로 만들면 타임라인 기반 애니메이션이 그대로 남으므로 `animation: none !important`로 확실히 해제했습니다.

#### **구조 (Phase 3)**
*   `body { overflow-x: hidden }`을 제거하고, 넘침을 만들던 장식 요소를 각 섹션의 `overflow: clip`으로 지역화했습니다. `clip`은 `hidden`과 달리 스크롤 컨테이너를 만들지 않아 내부 `position: sticky`를 망가뜨리지 않습니다. `.archive-section`의 기존 `overflow: hidden`도 `clip`으로 바꿨습니다.
*   격자를 `repeat(auto-fit, minmax(min(100%, 17rem), 1fr))`로 바꾸고 미디어 쿼리의 3→2→1열 단계 지정을 제거했습니다.
*   읽기 위주 섹션(intro, guestbook)의 `--section`을 760px로 좁히고 섹션 여백을 3단계 위계로 나눠, 9개 섹션이 같은 폭·같은 여백으로 반복되던 인상을 깼습니다.
*   모달 5개를 네이티브 `<dialog>` + `showModal()`로 전환했습니다. 포커스 트랩, 배경 inert, ESC 닫기, 최상위 레이어를 브라우저가 처리하므로 `HOMEPAGE_REVIEW.md` H-08이 함께 해소됩니다. 배경 클릭 닫기는 `event.target === dialog` 판정으로 유지하고, `close` 이벤트에서 스크롤 잠금을 해제해 ESC 경로도 일관되게 만들었습니다.
*   라이트박스에서 댓글 입력 중 좌우 방향키가 사진을 넘기던 문제(H-06)도 함께 고쳤습니다.

#### **계획 대비 변경**
*   `main` 명명 그리드 대신 기존 `--section` 변수 오버라이드를 썼습니다. 같은 결과를 훨씬 적은 회귀 위험으로 얻습니다.
*   `image-statement` 스티키 스크롤은 적용하지 않았습니다. 해당 섹션 카피가 키커+제목+뱃지뿐이라 스크롤될 내용이 없어 시각적으로 아무 일도 일어나지 않습니다.
*   히어로 이미지 패럴랙스도 제외했습니다. `.hero-course-card`가 히어로 경계 밖(`inset: -58px -70px auto auto`)에 있어 `overflow: clip`을 걸 수 없습니다.
*   모바일 메뉴 Popover 전환도 제외했습니다. 헤더 안에 인라인으로 펼쳐지는 구조라 최상위 레이어로 올리면 배치가 깨집니다.

#### **검증**
*   jsdom 렌더 검증으로 지원/미지원 두 경로 모두 확인. 지원 경로에서 `.reveal-ready` 0개(CSS 위임), 미지원 경로에서 48개(JS 폴백). 양쪽 콘솔 오류 0건, 모달 열기/닫기 정상.
*   미정의 CSS 토큰 참조 0건(런타임 주입 3종 `--progress`/`--animal-hue`/`--reveal-delay`는 모두 `var()` 폴백 보유).
*   `esbuild` CSS 파싱, `node --check`, 로컬 wrangler 서버 `/`·`/style.css`·`/main.js` 200 확인.
*   **미검증**: 실제 브라우저 시각 확인은 이 환경에 헤드리스 브라우저가 없어 수행하지 못했습니다. 특히 스크롤 기반 애니메이션 타이밍, View Transitions 실제 모션, `<dialog>` 전환 후 라이트박스 레이아웃은 배포 전 실기기 확인이 필요합니다.
